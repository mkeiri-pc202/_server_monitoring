...
- ユニットテスト（単体テスト）
  ...
  　   
- エンドツーエンドテスト
  - PC201からPC202に対して5分おきにステータスを送信し、データの送受信に問題が発生しないかをモニタした
    - クライアント側にPOSTのHTTPステータスコードが200以外になったログがあったか　→　なし
    -  サーバ側にHTTPステータスコードが200以外になったログがあったか　→　なし
    -  結果を表示した際に表に反映していないデータがあるか　→　なし（約５分おきにデータが登録されている）
    -  グラフが正常に表示されているか　→　グラフの横幅が狭くてわかりずらい、データ量が増えて横軸の時間の文字が重複してしまっているので１時間単位に変更する  
   
  - 負荷テスト
    - 複数台のPCでクライアントからのデータ送付でエラーがでるか　→ 試せていない
---
